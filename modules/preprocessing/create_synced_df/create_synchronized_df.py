...
location_file = os.path.join(raw_dir, 'Location.csv')
orientation_file = os.path.join(raw_dir, 'Orientation.csv')
image_folder = os.path.join(raw_dir, 'Camera')
...
# Define essential column names for the synchronized dataframe
essential_columns = [
    "gps_time",
    "date",
    "time",
...
# Drop all columns except the essential ones
fully_merged_df = fully_merged_df[essential_columns]

# Remove duplicates to ensure images only appear once
fully_merged_df = fully_merged_df.drop_duplicates(subset=['image'])

# Create output directory if it doesn't exist
output_dir = config.get_path('preprocessing', 'output_dir')
os.makedirs(output_dir, exist_ok=True)

# 5. Save synchronized dataframe to CSV
synced_df_path = config.get_path('preprocessing', 'synced_df_path')
os.makedirs(os.path.dirname(synced_df_path), exist_ok=True)
fully_merged_df.to_csv(synced_df_path, index=False)

print(f"Synchronized dataframe saved to {synced_df_path}")
